...
public class FileService {
    ...
    public void fileDownload(String filename, HttpServletResponse response ) {
        ...
        // 파일( byte) 읽어오기 --> 자바로 가져오기
        try {
            // 자바의 입장에서는 파일을 읽어서 사용자에게 파일다운로드 관련 응답해줘야 함.
            // 파일 입력 스트림 객체 생성
            FileInputStream fin = new FileInputStream( downloadPath );
            long fileSize = file.length(); // 파일 용량 바이트 수
            byte[] bytes = new byte[ (int)fileSize ];
            fin.read( bytes );
            fin.close(); // 스트림이란? 바이트가 다니는 이동 경로
            // ################ 다운로드 형식 : 브라우저마다 상이 ###################
            // 1. 파일의 uuid 제거
            String oldFilename = filename.split("_")[1]; // _ 언더바 기준으로 잘라서 2번째 문자열 뜻
            // 2, URL은 한글을 지원하지 않는다. 경로 인코딩 변환 : URLEncoder.encode(oldFilename, "UTF-8")
            // 주소에 검색키워드가 들어가는 경우
            response.setHeader("Content-Disposition", "attachment;filename = " + URLEncoder.encode(oldFilename, "UTF-8") );

            ServletOutputStream fout = response.getOutputStream();
            fout.write( bytes ); // 다운로드를 요청한 사용자에게 출력(쓰기)
            fout.close(); // 안전하게 stream 닫기 ==> 실무에서는 필수
    ...
    public boolean fileDelete( String filename ){
        String filePath = uploadPath + filename; // 삭제할 파일명과 업로드 경로 조합
